...
- **v2.0.0 (2026-09-02, "Çoklu Platform: macOS + Windows — Compose Multiplatform"):** §2'deki "yalnız Android tablet" varsayımı genişledi: aynı kod Kotlin Multiplatform ile macOS ve Windows'ta çalışır (`:shared` + ince `:androidApp`/`:desktopApp`; Hilt yerine Koin). Şema v5, yedek v5 ve `applicationId` değişmedi; yedek dosyaları platformlar arasında birebir taşınır. Platform sınırı tam olarak yedi sözleşme + Room kurucusu ve grafik bileşenleri için `expect/actual`'dır (ARCHITECTURE §12). Masaüstünde anahtarlar AES-GCM dosyada tutulur (Keystore'dan zayıf; Keychain/DPAPI ertelendi), sayaç bitişi süreç içi zamanlayıcı + tepsi bildirimi, paylaşım pano + kaydet diyaloğu, grafikler Canvas (Vico Android'de kaldı). Web (Wasm) ertelendi; Windows `.msi` yalnız CI'da üretilir; notarizasyon/imzalama ertelendi. Cihazlar ayrı adalardır — hesap/eşitleme yok (§1 gizlilik duruşu aynen).
- **v2.1.0 (2026-09-05, "Kardeş geri bildirimi"):** öğrencinin ilk geri bildirim turu. §7 Sayaç: SIFIRLA hiçbir şey kaydetmez, ekranda yalnız bugünün tamamlanmış oturumları; §6.4 Ana Sayfa halkası günlük (hafta not olarak); Planlayıcı'ya gün gün çalışma süresi; §15'teki konu-analizi genişlemesi: `topic_status`'a tekrar-gerekli bayrağı, öz-değerlendirme ve son çalışma zamanı (şema v6, ekleyerek; yedek v6). §8 AI Koç: bağlam artık uygulamanın tamamı (program gün gün, günlük odak, konu takibi, notlar) ve Claude profillerinde isteğe bağlı sağlayıcı-taraflı web araması (`ai_web_search`) — ağ yine yalnız AI ucuna gider.
